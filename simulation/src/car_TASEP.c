...
void Initial_Config_X(int L, int N, int *X, int *ptr);
void Initial_Config_Y(int L, int M, int *Y, int *ptr);
void Dynamics_Coupled(int L, int N, int M, int *X, int *Y, int *move_x, int *move_y, int *ptr);
void J_v(int L, int N, int M, int T, double *v_x0_mean, int *x0, double *v_x1_mean, int *x1, double *v_y0_mean, int *y0, double *v_y1_mean, int *y1, double *j_x, double *j_y, int *ptr);
...
void main(int argc, char *argv[])
{
  ...
  FILE *fp,*fp1,*fp2,*fp3,*fp4,*fp5;
    ...
    for(i=0;i<20;i++){
      ...
      J_v(L,N,M,T,&v_x0_mean,&x0[i][0],&v_x1_mean,&x1[i][0],&v_y0_mean,&y0[i][0],&v_y1_mean,&y1[i][0],&j_x,&j_y,ptr);
...
void J_v(int L, int N, int M, int T, double *v_x0_mean, int *x0, double *v_x1_mean, int *x1, double *v_y0_mean, int *y0, double *v_y1_mean, int *y1, double *j_x, double *j_y, int *ptr)
{
  ...
  int move_y_sum,move_x_sum;
    ...
    Initial_Config_X(L,N,X,ptr);
    Initial_Config_Y(L,M,Y,ptr);
    move_x=0;
    move_x_sum=0;
     *x0=0;
     *x1=X[1]-X[0];
    ...
    for(t=1;t<=T;t++){
      ...
      y0_ini=Y[0];
      y1_ini=Y[1];
      Dynamics_Coupled(L, N, M, X, Y, &move_x, &move_y, ptr);
      move_x_sum+=move_x;
      move_y_sum+=move_y;
